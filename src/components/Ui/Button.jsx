...
export default function Button({
  ...
  const baseStyles =
    "inline-flex items-center justify-center font-medium transition-all duration-150 rounded-lg select-none disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer focus:outline-none shrink-0 min-w-0";

  // 2. Mobile Full Width Handler
  const widthStyle = fullWidthOnMobile ? "w-full sm:w-auto" : "";

  // 3. Responsive Color Variants
  const variants = {
    primary:
  ...
  const sizes = {
    sm: "px-2 py-1 text-[10px] sm:text-[10.5px] gap-1",
    md: "px-2.5 sm:px-3 py-1.5 text-[10.5px] sm:text-[11px] gap-1.5",
  ...
  return (
    ...
    >
      ...
      {/* Text with Truncate Safety for Mobile */}
      {children && (

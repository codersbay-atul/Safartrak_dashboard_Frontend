...
export default function Button({
  ...
  const variants = {
    primary: "bg-[#FDBB24] text-black hover:bg-[#e0a31f] active:scale-[0.98]",
  ...
  return (
    <button
      ...
      {...props}
    >
      {Icon && iconPosition === "left" && <Icon size={iconSizes[size] || 12} className="shrink-0" />}
      {children && <span className="truncate max-w-full leading-none">{children}</span>}
      {Icon && iconPosition === "right" && <Icon size={iconSizes[size] || 12} className="shrink-0" />}
    </button>
  );
}

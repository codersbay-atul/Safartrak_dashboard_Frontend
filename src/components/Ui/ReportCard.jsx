...
export default function ReportCard({
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-3 text-[12.5px] sm:text-[13px] font-bold text-white tracking-tight leading-tight">{title}</h3>
      <p className="mt-1.5 text-[10px] sm:text-[10.5px] text-[#a1a1aa] leading-relaxed line-clamp-2 min-h-[2.5rem]">{description}</p>
      <div className="mt-auto pt-3.5 flex items-center justify-between gap-2">
        <span className="text-[10.5px] sm:text-[11px] font-semibold text-[#FDBB24] group-hover:text-[#E9AE17] transition-colors">{actionLabel}</span>
        <ArrowUpRight size={14} strokeWidth={2.25} className="text-[#FDBB24] shrink-0 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
      </div>
    </button>
  );
}

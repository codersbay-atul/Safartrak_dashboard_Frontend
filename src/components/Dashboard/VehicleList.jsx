...
export default function VehicleList({ onSelectVehicle }) {
  ...
  return (
    <div className="w-full h-full bg-[#121214] border border-[#1f1f23] rounded-xl p-3 flex flex-col select-none overflow-hidden">
      ...
      {/* Title */}
      <h3 className="text-[12px] font-bold text-white tracking-tight mb-1.5 shrink-0">
        Vehicle List
      </h3>
      ...
      <div className="flex flex-col gap-1.5 overflow-y-auto pr-0.5 flex-1 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
        {vehicles.map((vehicle) => {
          ...
          return (
            <div 
              key={vehicle.id} 
              className={`flex items-center justify-between py-2 px-2.5 sm:py-1.5 sm:px-2 rounded-lg border transition-all duration-200 w-full gap-3 shrink-0
                ${isSelected 
            ...
            >
              ...
              <div className="min-w-0 flex-1 flex flex-col justify-center">
                <div className="flex items-center gap-1.5 leading-none flex-wrap sm:flex-nowrap">
                  <span className="text-[11px] sm:text-[10.5px] font-bold text-white tracking-tight shrink-0">
                    {vehicle.plate}
                  </span>
                  <span className="text-[8px] text-zinc-600 hidden sm:inline">•</span>
                  <span className={`text-[9px] sm:text-[8.5px] font-semibold ${vehicle.statusColor} truncate`}>
                    {vehicle.status}
                  </span>
                </div>
                
                <div className="flex items-center gap-1 text-[8.5px] sm:text-[8px] text-zinc-500 truncate mt-1 max-w-[140px] sm:max-w-[120px]">
              ...
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                <div className="text-right leading-none">
                  <p className="text-[10px] sm:text-[9.5px] font-bold text-white">
                    {vehicle.speed}
                  </p>
                  <p className="text-[8px] sm:text-[7.5px] text-zinc-500 mt-0.5 max-w-[55px] truncate">
                    {vehicle.location}
                  </p>
                </div>
                ...
                <button 
                  ...
                  className={`h-6 sm:h-5 px-2.5 sm:px-1.5 rounded text-[9px] sm:text-[8.5px] font-bold transition-all shrink-0 cursor-pointer flex items-center justify-center
                    ${isSelected
